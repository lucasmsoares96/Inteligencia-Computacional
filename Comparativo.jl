...
md"""
# Comparativo de Algoritmos para Previsão de Bolsa de Valores
"""
...
# ╔═╡ 6f00abff-55da-4fe3-8bd9-c6a1f0cf1827
acao2 = xlsx[:, 2]

# ╔═╡ 8efd8318-2d87-4451-b97f-0ad077051052
begin
	Xa, ya = Utils.tSeriesVal(acao1)
	DataFrame(Xa, :auto)
end

# ╔═╡ 9843d28c-f17e-4f1c-895e-5b161f612b37
ya

# ╔═╡ 86926995-8ce5-47c3-a218-e07a532de5cb
dfa = DataFrame(Xa, :auto)[:, ["x32", "x33", "x34", "x35", "x36"]]

# ╔═╡ f19723be-77c0-4992-aa41-e7e78a4c4ada
begin
	Xbl, ybl = Utils.tSeriesVal(acao2)
	ybl
end

# ╔═╡ be326e70-de36-4868-8ed1-5af6788dc420
dfbl = DataFrame(Xbl, :auto)[:, ["x32", "x33", "x34", "x35", "x36"]]

# ╔═╡ de29e904-2df1-4d04-8059-da09f97ed010
begin
	Xbr, ybr = Utils.tSeriesVar(acao2)
	DataFrame(Xbr,:auto)
end

# ╔═╡ ee4e8878-75f0-45f2-a4e6-4a25309a51bc
ybr

# ╔═╡ 5c283787-e146-4e50-9f5d-be51586de565
dfbr = DataFrame(Xbr, :auto)[:, ["x1", "x11", "x14", "x16", "x18"]]

# ╔═╡ 81654091-c999-4156-8f45-7f326fb2f35b
mbr = dfbr |> Matrix

# ╔═╡ e864020e-f467-4618-89be-236a6b0f7d3d
y_atual = acao2[20:end-1]

# ╔═╡ 6cff29dd-ec06-4e40-ac23-d910da8e52de
md"""
## Estratégia de Buy and Hold
"""

# ╔═╡ 446db6d7-0d54-4c21-80c0-bd78676bd890
buyHold = repeat([0.1], length(y_atual))

# ╔═╡ 1de01861-d23a-4b39-80e5-22aae3178fe1
sbuyHold, cont = Utils.simulation(; initValue=10000, predict=buyHold, real=y_atual)

# ╔═╡ 9a6684aa-8a11-4f79-ad3d-82d4602b4e39
cont

# ╔═╡ 3af0683d-df95-4abe-aa85-f0fbc7082a65
last(sbuyHold)

# ╔═╡ e9fdf14e-c109-4a7e-aa18-f0428c3b83f8
plot(
    plot([ybr[2:end], buyHold[2:end]];
        label=["Real" "Previsto"],
        xlabel="Operações",
        ylabel="Variação"
    ),
    plot(y_atual;
        xlabel="Operações",
        ylabel="Valor da Ação"
    ),
    plot(sbuyHold;
        xlabel="Operações",
        ylabel="Valor Aplicado"
    );
    layout=(3, 1),
    plot_title="Compra e Espera"
)

# ╔═╡ aa4fdea3-df0e-4ca5-8230-ca38d8c17283
md"""
## Algoritmo Neo-Fuzzy-Neoron
"""

# ╔═╡ a47c195d-1610-4415-98a7-f15c62ecd0cb
q = 2

# ╔═╡ ba92412d-70ec-40c0-9c46-85c11363e0c4
tmin, tmax = extrema(mbr)

# ╔═╡ 5f591813-8079-4496-a753-238c1eda551e
t = tmin:0.005:tmax

# ╔═╡ 48597601-aa86-44bb-b2aa-a0e76215ba46
func = NFN.defFunc(mbr, q)

# ╔═╡ c89ba126-8ddc-4a19-b553-86152c1ca266
NFN.plotFunc(func, t)

# ╔═╡ 586c27de-ece9-47b1-b838-63cb9a7453e7
nfn = NFN.predict(mbr, ybr, func)

# ╔═╡ 93b2a003-df04-4ae5-87f9-1259392ecc49
snfn, cnfn = Utils.simulation(; initValue=10000, predict=nfn, real=y_atual)

# ╔═╡ 642ee070-4a82-4e3b-a916-2930cced7b4a
cnfn

# ╔═╡ c0c24a49-32a8-41c3-a213-08baf9953d5f
last(snfn)

# ╔═╡ 279d1cc2-def3-45c2-8524-b4df0544430e
plot(
    plot([ybr[2:end], nfn[2:end]];
        label=["Real" "Previsto"],
        xlabel="Operações",
        ylabel="Variação"
    ),
...
# ╔═╡ 7547b5ae-e889-4457-85c2-84a63d359afe
md"""
## Algoritmo Neural Network
"""

# ╔═╡ 1926ec2b-c667-4420-a030-9be1a576cf94
mach = machine("mach.jlso")

# ╔═╡ 70e57886-c2de-402a-99f7-11b59f914439
nn = MLJ.predict(mach, dfbl)

# ╔═╡ d187151d-4abf-4d35-a41d-ad78c19f07bb
nnVar = nn .- y_atual

# ╔═╡ 2c06675a-7296-469e-acab-6bab1fb94133
snn, cnn = Utils.simulation(; initValue=10000, predict=nnVar, real=y_atual)

# ╔═╡ 28a40211-9526-4b62-8676-09dd070dcb23
cnn

# ╔═╡ 42a5a116-e145-4969-8d33-e5b47095b81d
last(snn)

# ╔═╡ 60362c88-2130-473c-a83a-121384bc5d90
plot(
    plot([ybl[2:end], nn[2:end]];
        label=["Real" "Previsto"],
        xlabel="Operações",
        ylabel="Valor da Ação"
    ),
...
# ╔═╡ b587f1d2-cccc-4a75-85af-ba3c1fc0cf7c
md"""
## Resultados
"""

# ╔═╡ 0b54f2d8-7d42-4cea-b258-a5f54ae30d90
plot(
    plot([snn, snfn, sbuyHold];
        label=["NN" "NFN" "B&H"],
        xlabel="Operações",
        ylabel="Valor Aplicado"
    );
    plot_title="Comparativo"
)

# ╔═╡ 78da5602-b617-49b0-8293-986c8af661ed
MLJ.rmse(buyHold, ybr)

# ╔═╡ cc986f6d-3d2c-4058-8a86-f84df0c17dcd
MLJ.rmse(nfn, ybr)

# ╔═╡ 1a320b93-c10e-4e74-9dbd-fde87145953f
MLJ.rmse(nn, ybl)
